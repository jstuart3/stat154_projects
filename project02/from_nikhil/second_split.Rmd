---
title: "Second Method of Splitting Data"
...
```{r}
split1_train = read_csv("./split2_train.csv")
split1_val = read_csv("./split2_val.csv")
test = read_csv("./split2_test.csv")
```
...
```{r}
ggplot(data = log_roc, aes(x= FPR, y = TPR)) + geom_point() + ggtitle("ROC of Logistic Regression")
```
...
```{r}
ggplot(data = lda_roc, aes(x= FPR, y = TPR)) + geom_point() + ggtitle("ROC of LDA")
```
```{r}
...
```{r}
ggplot(data = qda_roc, aes(x= FPR, y = TPR)) + geom_point() + ggtitle("ROC of QDA")
...
```{r}
ggplot(data = tree_roc, aes(x= FPR, y = TPR)) + geom_point() + ggtitle("ROC of Decision Tree")
```
...
##a.
A good classification model found was logistic regression on the second split of data. Now, I will attempt to optimize logistic regression by seeing if there is a more optimal point that 0.5 to use as the decision boundary for the two classes.
...
```
Just like in the first split of data it seems that a discrimination Percent of 0.4 is slightly better than 0.5. However, for the same reason, we will ignore this anomaly. Now lets see if we can reduce test_error with L1-regularization.
...
Again, lets graphically analyze the different lambda values and visually determine approximately where a good lamda value would be. 
...
print(log_loss)
```
Here we see that even with the L1-regularization we still get approximately the same error rate as before without it. It seems that neither L1-regularization nor changing the discrimination percentage helped much with lowering the test error rate. 

##b.
```{r}
confusionMatrix(as.factor(log_pred[,1]), as.factor(test$expert_label))
```
The Confusion Matrix and summary statistics above give an analysis of the misclassification error of the logistic regression model. There seems to be an even split of false negatives and false positives indicating no tendency of the model towards either type of error. This logisitic regression model on the second split of data has a much higher specificity (about double) that of the model created by the first split of data while only having .06 less sensitivity. It is clear that this second model is the superior one. 


##c/d.
In conclusion, my analysis on parts a and b was largely fruitless for both splits of data. Although I made several interesting findings regarding my model (that a discriminative value of 0.4 was optimal to 0.5, and the optimal regularization paramter for L1-regularization), I failed to find anything that largely decreased my model's test error rate. Conversly, adding L1-regression actually largely increased test error for the first data split. On the positive side though, I believe the logistic regression model will work pretty well on future data. For the model made on the second split of data, on 95% of sets of test data, the prediction accuracy will be between 88.45& and 89.31%. This is a strong accuracy number. Overall the second split of data proved to be a much better split in terms of predictive power. All models realized a lower test error rate in the second split than the first. 